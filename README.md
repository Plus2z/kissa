...
**Kissa（喫茶）** 取名自日文中的“喝茶、咖啡馆”，灵感源于我们对一段惬意、有序对话的想象——写终端也可以像坐在喫茶店里品一杯咖啡那样从容。它不是为了颠覆世界而造的严肃生产力怪物，纯粹是出于好玩（Just for fun），试图把黑底白字的冰冷滚动流，变成一场赏心悦目的“人机茶谈”。

### 核心特色

- **气泡式会话交互**：告别杂乱的无限文本瀑布流。通过旁路状态机与 Shell 深度集成，命令、输出、耗时统计与退出状态被重构为清晰易读的独立对话气泡，每一次敲击回车都像在发送一条消息。
- **真实终端作为唯一事实来源（Source of Truth）**：不同于浮于表面的包装器，Kissa 的底层 PTY 字节流在后台完整保真运转，不篡改、不吞行、不丢数据。气泡仅仅是附着其上的“无害旁路增强”；遇到复杂全屏程序或未知模式时，支持随时切回标准终端视图，提供 100% 可靠的硬降级保证。

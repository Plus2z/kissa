...
### 阶段六(生产力增强与交互打磨)
- **对话记录导出**:顶栏与 `/export` 支持将当前会话历史一键导出为包含命令、路径、退出码及耗时的 Markdown 文档
- **对话流全文检索**:顶栏搜索按钮或 `Ctrl+F`/`Cmd+F` 调出搜索浮层，实时高亮匹配历史命令与输出，支持上下键平滑跳转
- **快捷指令与 Slash 命令**:输入框支持 `/clear`、`/export`、`/ssh`、`/help` 等快捷指令，并在空输入时提供常用命令快捷气泡
- **终端字体定制**:设置面板支持切换专业等宽字体（JetBrains Mono / Fira Code / Cascadia Code / Source Code Pro 等），实时生效并持久化
- **气泡快捷交互**:命令气泡悬停支持一键复制与快速重填，支持文件拖拽到输入框自动转义填入路径
